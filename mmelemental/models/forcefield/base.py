...
class ForceField(Base):
    ...
    bonds_style: Optional[str] = Field(None, description = 'Bond potential form e.g. harmonic, morse, etc.')

    angles: Optional[Tuple[int, int, float]] = Field(None, description = 'Angle parameters.')
    angles_style: Optional[str] = Field(None, description = 'Angle potential form e.g. harmonic, quartic, etc.')

    proper_dihedrals: Optional[Tuple[int, int, int, int, float]] = Field(None, description = 'Dihedral/torsion parameters.')
    proper_dihedrals_style: Optional[str] = Field(..., description = 'Proper dihedral potential form e.g. harmonic, helix, fourier, etc.')

    improper_dihedrals: Optional[Tuple[int, int, int, int, float]] = Field(..., description = 'Dihedral/torsion parameters.')
    improper_dihedrals_style: Optional[str] = Field(..., description = 'Improper dihedral potential form e.g. harmonic, fourier, etc.')

    charges: Array[float] = Field(..., description = 'Atomic charges.')
    nonbonded_style: str = Field(..., description = 'Non-bonded short potential form e.g. Lennard-Jones, Buckingham, etc.')
