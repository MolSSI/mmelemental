...
class ForceField(Base):
    bonds: Optional[Array[Array[float]]] = Field(None, 
        description = 'Bond parameters: eq length, constant (e.g. spring const), extra params.')
    bonds_type: Optional[List[str]] = Field(None, description = 'Bond potential form e.g. harmonic, morse, etc.')

    angles: Optional[Array[Array[float]]] = Field(None, 
        description = 'Angle parameters: eq angle, constant (e.g. spring const), extra params.')
    angles_type: Optional[List[str]] = Field(None, description = 'Angle potential form e.g. harmonic, quartic, etc.')

    dihedrals: Optional[Array[Array[float]]] = Field(None, 
        description = 'Dihedral/torsion parameters: eq energy, phase, extra params.')
    dihedrals_type: Optional[List[str]] = Field(None, description = 'Proper dihedral potential form e.g. harmonic, helix, fourier, etc.')

    improper_dihedrals: Optional[Array[Array[float]]] = Field(..., description = 'Dihedral/torsion parameters.')
    improper_dihedrals_type: Optional[List[str]] = Field(None, description = 'Improper dihedral potential form e.g. harmonic, fourier, etc.')

    charges: Array[float] = Field(..., description = 'Atomic charges.')
    charge_groups: Optional[Array[int]] = Field(None, description = 'Charge groups per atom.')

    nonbonded: Array[float] = Field(..., description = 'Non-bonded short potential parameters.')
    nonbonded_type: Optional[List[str]] = Field(None, description = 'Non-bonded short potential form e.g. Lennard-Jones, Buckingham, etc.')
    ...
    types: Optional[List[str]] = Field(None, description = "Atom types e.g. HH31. The type names are associated with the atomic \
        elements defined in other objects e.g. see the :class:``Molecule`` model.")

    @classmethod
    def from_file(cls, filename: Union[FileInput, str], dtype: Optional[str] = None, 
        **kwargs) -> "ForceField":
        """
...
class FFReaderComponent(GenericComponent):
    ...
    def execute(
        ...
        if inputs.data:
            ...
            raise NotImplementedError('Forcefield can be instantiated only from files or other ssupported data objects.')